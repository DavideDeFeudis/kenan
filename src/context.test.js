...
import { localWorkshops, onlineCourse } from "./data";
...
describe("reducer", () => {
  ...
  it("case GET_SUCCESS", () => {
    expect(
      reducer(state, { type: GET_SUCCESS, payload: localWorkshops })
    ).toEqual({
      ...initialState,
  ...
  it("case SET_PREVIEW", () => {
    expect(
      reducer(state, { type: SET_PREVIEW, payload: localWorkshops[0] })
    ).toEqual({
      ...state,
      createStatus: "",
  ...
  it("case OPEN_MODAL_WORKSHOP", () => {
    expect(
      ...
      modalItem: state.workshops.find(
        (workshop) => workshop._id === "81b0b3681cf6"
      ),
      isModalOpen: true,
    });
  });

  it("case OPEN_MODAL_ONLINE_COURSE", () => {
    expect(
      reducer(state, {
